...
namespace XJHSelfUse
{
    class UserInfo
    {
        ...
        {
            FileStream file = FileSelect.CreateNewOrTruncate(Path.GetFullPath("UserInfo.txt"));
        ...
        public static void SaveUserInfo(List<string> str)
        {
            FileStream file = FileSelect.CreateNewOrTruncate(Path.GetFullPath("UserInfo.txt"));
            StreamWriter txt_w = new StreamWriter(file);
            foreach (string s in str)
        ...
        public static bool ReadUserInfo(out List<string> str)
        {
            if (File.Exists("UserInfo.txt"))
            {
                ...
                str = new List<string>();
                string tmp;
                while ((tmp = file_r.ReadLine()) != null)
                {
                    str.Add(tmp.Trim());
                }
                file_r.Close();
        ...
        {
            FileStream file = FileSelect.CreateNewOrTruncate("DebugInfo.txt");
